...
protected:
   ...
   struct TreeNode {
      ...
      Link iz, dr;
      int altura;
	  int tam_i; //num nodos en hijo izq + 1 (este nodo).
...
public:
	void resolver(std::vector<int>const & consultas) {
		for (int i = 0; i < consultas.size(); ++i) {
			if (consultas[i] > this->size())
				std::cout << "??" << '\n';
			else std::cout << kesimoelem(this->raiz, consultas[i]) << '\n';
		}
   }
	...
	int kesimoelem(Link r, int k) {
		if (r == nullptr)
			...
			if (r->tam_i == k)
				...
				if (r->tam_i < k)
					return kesimoelem(r->dr, k - r->tam_i);
				else
					return kesimoelem(r->iz, k);
			}
		}
	}
